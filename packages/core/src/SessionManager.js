 ...
 */
// ─── synchronous SHA-256 (pure JS, no deps) ──────────────
...
export function sha256Hex(input) {
    ...
    const bitLen = l * 8;
    // padding to 64-byte blocks with 8-byte length tail
    const withLen = (((l + 8) >> 6) + 1) << 6;
    const buf = new Uint8Array(withLen);
    buf.set(bytes);
    buf[l] = 0x80;
    // 64-bit big-endian length (high word stays 0 for our small inputs)
...
export class SessionManager {
    ...
    generateSegmentNonce(sessionId, segmentIndex) {
        return sha256Hex(`${sessionId}:${segmentIndex}`);
    }
    /**
     * Returns true if this nonce has already been used (replay). First use is
     * recorded and returns false.
     */
    isReplay(nonce, _sessionId, ttlSeconds = 120) {
        this.evictExpired();
     ...
     */
    validateNonce(nonce, sessionId, segmentIndex, ttlSeconds = 120) {
        const expected = this.generateSegmentNonce(sessionId, segmentIndex);
        if (nonce !== expected)
            return false;
        return !this.isReplay(nonce, sessionId, ttlSeconds);
    }
    evictExpired() {
        const now = Date.now();
